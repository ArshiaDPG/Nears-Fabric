...
public class NearsBlockTagProvider extends FabricTagProvider<Block> {
    ...
    protected void configure(RegistryWrapper.WrapperLookup arg) {
        ...
        getOrCreateTagBuilder(NBlockTags.NEAR_HANG_PLANTABLE_ON).add(Blocks.CRIMSON_NYLIUM).add(Blocks.NETHER_WART_BLOCK);

        getOrCreateTagBuilder(NBlockTags.FAAR_GROWTH_PLANTABLE_ON).add(Blocks.WARPED_WART_BLOCK);

        getOrCreateTagBuilder(NBlockTags.FAAR_CLUSTER_REPLACEABLE).add(Blocks.AIR);
        ...
        getOrCreateTagBuilder(NBlockTags.CINDER_GRASS_PLANTABLE_ON)
                ...
                .forceAddTag(BlockTags.NYLIUM).forceAddTag(NBlockTags.CINDER_WHEAT_PLANTABLE_ON).add(Blocks.NETHERRACK);

        getOrCreateTagBuilder(NBlockTags.CINDER_WHEAT_PLANTABLE_ON).add(Blocks.MAGMA_BLOCK).add(Blocks.BASALT);

        getOrCreateTagBuilder(NBlockTags.SUMMER_CROPS_COMPAT)
                .add(NBlocks.SOUL_BERRY_BUSH)
                .add(NBlocks.FAAR_GROWTH)
                .add(NBlocks.CINDER_GRAIN)
                .add(NBlocks.NEAR_HANG)
                .add(NBlocks.NEAR_HANG_STEM);
    }
}
